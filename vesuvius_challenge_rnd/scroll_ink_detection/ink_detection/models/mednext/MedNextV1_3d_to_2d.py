...
from vesuvius_challenge_rnd.scroll_ink_detection.ink_detection.models.mednext.blocks import (
    MedNeXtBlock,
    MedNeXtDownBlock,
    MedNeXtUpBlock,
    OutBlock,
...
class MedNeXt3dto2d(nn.Module):
    def __init__(
        ...
        self.out_0 = OutBlock(in_channels=n_channels, n_classes=n_classes, dim="2d")

        # Used to fix PyTorch checkpointing bug
        self.dummy_tensor = nn.Parameter(torch.tensor([1.0]), requires_grad=True)

        if deep_supervision:
            self.out_1 = OutBlock(in_channels=n_channels * 2, n_classes=n_classes, dim="2d")
            self.out_2 = OutBlock(in_channels=n_channels * 4, n_classes=n_classes, dim="2d")
            self.out_3 = OutBlock(in_channels=n_channels * 8, n_classes=n_classes, dim="2d")
            self.out_4 = OutBlock(in_channels=n_channels * 16, n_classes=n_classes, dim="2d")
    ...
    def forward(self, x):
        ...
        if self.outside_block_checkpointing:
            ...
            # Depth pool.
            x_res_0 = x_res_0.max(axis=2)[0]
            x_res_1 = x_res_1.max(axis=2)[0]
            x_res_2 = x_res_2.max(axis=2)[0]
            x_res_3 = x_res_3.max(axis=2)[0]
            x = x.max(axis=2)[0]
