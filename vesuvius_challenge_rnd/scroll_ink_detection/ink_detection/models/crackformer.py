...
class Mlp(nn.Module):
    def __init__(self, in_features, out_features, act_layer=nn.GELU, drop=0.0, linear=False):
        super().__init__()
...
class LocalSABlock(nn.Module):
    ...
    def forward(self, x):
        ...
        softmax = self.softmax(
            self.keys(x).view(n_batch, self.kk, self.uu, w * h)
        )  # b, k, uu, w * h
...
class TFBlock(nn.Module):
    def __init__(
        ...
        self.attn = LocalSABlock(in_channels=in_chnnels, out_channels=out_chnnels)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        self.mlp = Mlp(
            in_features=in_chnnels,
            out_features=out_chnnels,
            act_layer=act_layer,
            drop=drop,
            linear=linear,
        )
...
class Bottleneck(nn.Module):
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.bn1 = nn.GroupNorm(hidden_planes // 4, hidden_planes)
        self.conv2 = nn.ModuleList([TFBlock(hidden_planes, hidden_planes)])
        self.bn2 = nn.GroupNorm(hidden_planes // 4, hidden_planes)
...
class LABlock(nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        self.W_1 = nn.Sequential(
            nn.Conv2d(
                input_channels, output_channels, kernel_size=3, stride=1, padding=1, bias=True
            ),
            nn.GroupNorm(output_channels // 4, output_channels),
        )
        self.psi = nn.Sequential(
            nn.Conv2d(
                output_channels, output_channels, kernel_size=3, stride=1, padding=1, bias=True
            ),
            nn.GroupNorm(output_channels // 4, output_channels),
            nn.Sigmoid(),
        )
...
class Down1(nn.Module):
    def __init__(self, in_channels: int = 3):
        super().__init__()
        self.nn1 = ConvRelu(in_channels, 64)
...
class Crackformer(nn.Module):
    def __init__(self, in_channels: int = 1, out_channels: int = 1):
        ...
        self.down1 = Down1(in_channels=in_channels)
        self.down2 = Down2()
        self.down3 = Down3()
        self.down4 = Down4()
        self.down5 = Down5()
        ...
        self.final = Conv1X1(5, out_channels)
